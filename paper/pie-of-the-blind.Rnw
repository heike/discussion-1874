...
Let's assume that the actual proportion of blind individuals in the US at 1870 is actually 241.5 in 443,377 \hh{XXX based on total \# blind}.  Then $X$, the number of blind individuals in the microsample, follows a Binomial distribution $B_{n, p }$ with $\hat{p} = 20970.51/(38.5\cdot 10^6)$ and $n = 443377$. This means that $X$ has an expected value of 241.5, but its variance is the same size (Var$(X) = 241.4)$, which means that a Wald 95\% confidence interval is (222, 282). This puts the relative uncertainty around the number of blind individuals at about 25\%.
