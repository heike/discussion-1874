...
\subsection{What is the chart supposed to tell us?}
\hh{

Interpretation by Wines (p.3 of the introduction) in his own words:
\begin{itemize}
\item ``Males are shown to be in excess among the Blind"
\item ``Foreigners are shown to be in excess of their proportion among the Blind"
\item ``The Colored are shown to be in excess of their proportion among the Blind"
\end{itemize}

Based on the chart he presented, it is not immediately clear, how these statements come to pass.
}

\subsection{What can the chart tell us?}
...
state_pop <- subset(state_pop, State_Caps != "COLORADO") # not a state until 1876
...
\hh{Obviously, a direct comparison of the numbers by state allows us to better quantify the correlation between the variables involved. The scatterplot in Figure~\ref{fig:correlation} shows the strong relationship between the number of blind and total population for each state (the correlation is \Sexpr{round(with(state_pop, cor(TOTAL.POPULATION, blindEst, use="complete.obs")),3)}). States with a higher number of blind individuals compared to total population of the state are labelled. Figure~\ref{fig:rates-overall} shows a choropleth map of the rate of blind per 10,000 that reveals a fairly strong geographic pattern of relatively many blinds in states in the South East. }
% \ak{We're talking about correlation here, would a scatter plot make sense to include also?} 
% \hh{XXX I agree, it does make sense - showing correlation on a map is not ideal. }
\begin{figure}
...
ggplot(aes(x=TOTAL.POPULATION/10^6, y=blindEst), data = state_pop) +
  ...
  theme_bw() +
  geom_label(aes(label=state.abb), nudge_x=-.175, alpha=0.5, size=3,
             data=subset(state_pop, 
                         blindEst-blindPred>100 & state.abb != "TN")) + 
...
@
\caption{\label{fig:rates-overall}Choropleth map of the number of blind per every 10,000 individuals. A fairly strong geographic relationship becomes apparent: highest rates of blinds are in states surrounding the capital.}
\end{figure}
...
\hh{XXX motivate p more XXX}
Let's assume that the actual proportion of blind individuals in the US at 1870 is actually 241.5 in 443,377 \hh{XXX based on total \# blind}.  Then $X$, the number of blind individuals in the microsample, follows a Binomial distribution $B_{n, p }$ with $\hat{p} = 20970.51/(38.5\cdot 10^6)$ and $n = 443377$. This means that $X$ has an expected value of 241.5, but its variance is the same size (Var$(X) = 241.4)$, which means that a Wald 95\% confidence interval is (222, 282). This puts the relative uncertainty around the number of blind individuals in the microsample at about 25\%.

For blind individuals at the state level the situation is worse and we should therefore not base inferential statements about blind individuals  on the microsample.
...
While the 1.2\% microsample does not help with  the small number of blind at the state level, it does allow us to get an estimate for the total number of blind for the nation: using the 252 records on blind individuals in the 1.2\% microsample as a basis, we get (using the survey weights associated with each record) an estimated number of  20970.51 blind individuals nationwide with a corresponding 95\% Wald confidence interval of (20687, 21254). This is the number that the values in Figure~\ref{fig:mapblind} is based on.


\section{Fact checking}
\hh{Following along Wines' statements we will provide graphical evidence to check on his statements and also comment on their visual accessibility. }
\subsection{``Males are shown to be in excess among the Blind"}
\hh{XXX when talking about excess, the baseline needs to be defined. for gender we have a natural assumption of 50:50, which we can use here. }
\hh{XXX this statement turns out to be ok both at face value (i.e. assuming a 50:50 ratio of the genders) and when adjusting for the state marginal distributions. The face value can be seen in the pie charts when adding the two segments for men. }
\hh{XXX We also see from the topleft pie for the whole of the U.S.\ that the proportion of men is (quite a bit) more than 50 percent.}
\subsection{``Foreigners are shown to be in excess of their proportion among the Blind"}
\hh{XXX Unlike the previous statement there is no natural baseline, so there is no way to assess this statement based on Wines' pie charts, i.e. if we think of these statements by Wines as the caption of the chart, it does not match with the chart, which is always confusing to the audience \cite{robbins2005}. 
here, we can include a chart that compares the numbers.
The statement turns out to be true on the proportions, but we should also include the number of people that it is based on (because different states have very different population sizes). }
\subsection{``The Colored are shown to be in excess of their proportion among the Blind"}
\hh{XXX Again, there is no natural baseline, so we cannot use the pie charts to assess the validity of the statement. Based on the actual numbers this statement is true for some states, but not for all. XXX Need to still check the overall numbers. This statement is in direct conflict to our finding later on - that is a Simpson's paradoxon~\cite{simpson1951}. }
...
\hh{XXX some of these findings are in direct contradiction to Wines' statements. That's probably du to a Simpson's paradox. Let's pull out the example of Texas and go through the numbers. }
...
gborn <- as.data.frame(xtabs(data=ipums, SLWT~STATEICP+SEX+I(NATIVITY=="Foreign born")))
gborn$Freq <- gborn$Freq/100
...
Figure~\ref{fig:map-resids-race} tells maybe the most important story of the blind population of the US in 1870. Unfortunately, it is the story of discrimination within a minority. From the pattern of deviations in the choropleth maps we see that in most states white men among the blind are  over-represented, while all other demographics, and in particular white females are under-represented in most states. For men and women of color, the geographic pattern is more random, but overall there are more states, in which these two groups are under-represented than over-represented.
\begin{figure}
<<ipums-gender-race, dependson='simulate-resid-distr', echo=FALSE>>=
grace <- as.data.frame(xtabs(data=ipums, SLWT~STATEICP+SEX+I(RACE=="White")))
grace$Freq <- grace$Freq/100.0
...
@
\caption{\label{fig:map-resids-race} Choropleth maps of the deviations of observed and expected proportions of blind by gender and race show the real story in the data: there are significantly too many white men among the blind in most states. White females are under-represented in almost all states. Non-white blinds are under-represented in more states than not.}
\end{figure}
...
@
\caption{\label{fig:map-change}Choropleth map of a geographic overview of the changes in the number of blind between 1860 and 1870. \hh{XXX how do these changes compare to whole population?}}
\end{figure}

\section{Other things}
One of the reasons why foreign borns might be `in excess' of their proportion is age: there is a clear correlation between age and nativity. Foreign-born individuals are generally older. The age of blind individuals exhibits a clear double mode: the first mode is around age 15-20, corresponding to the age distribution of the young nation. Another mode in the age of blind b=individuals can be found in the older population -- blindness is clearly associated with losing eye-sight at an advanced age. 
We would need to test whether the percentage of foreigners among the blind is still too high after adjusting for age.
<<echo=FALSE>>=
ipums$native <- ipums$NATIVITY != "Foreign born"
qplot(AGE, weight=SLWT/100, data=ipums, fill=native) + facet_grid(BLIND~., scales="free_y", labeller="label_both") 
@
