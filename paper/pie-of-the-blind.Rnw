...
\hh{XXX Unlike the previous statement there is no natural baseline, so there is no way to assess this statemnt based on Wines' pie charts. here, we can include a chart that compares the numbers.
