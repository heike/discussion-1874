...
The deviations (in form of relative residuals) are plotted on choropleth maps as shown in Figure~\ref{fig:map-resids}. We see that the largest negative residuals (i.e.\ most underrepresented proportion of blind) are  native born females. There are far fewer native born blind women than expected. The most over-represented group are foreign born blind men, who in almost all of the states make up a by far larger number than expected.
