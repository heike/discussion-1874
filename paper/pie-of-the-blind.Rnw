...
<<ipums-gender-nativity, echo=FALSE>>=
...
blind.plus2$residCut <- cut(blind.plus2$resid, breaks=c(-20,-2.6,-2,-1.65, 1.65, 2, 2.6, 20)/2)
## this needs some more work

ggplot(aes(fill=residCut), data=blind.plus2) + 
  geom_polygon(aes(x=long, y=lat, group=group), fill=NA, colour="grey90", data=states) +
  geom_map(aes(map_id=region), map=states) +
  expand_limits(x=states$long, y=states$lat) +
  facet_wrap(~gender.born) + 
  scale_fill_brewer("(obs - exp)/sqrt(exp)", palette="RdYlBu") + #, na.value = "grey", limits=c(-2.5,2.5)) +
  theme_map() + 
  theme(legend.position="bottom")
@

The deviations (in form of absolute residuals) are plotted on choropleth maps below. We see that the largest negative residuals (i.e. most overestimated proportion of bline) are the native born female. There are far fewer native born blind women than expected. The most under-estimated group are foreign born blind men, who in almost all of the states make up a larger number than expected.
